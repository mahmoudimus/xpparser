 ...
 */
public class XPathXPrinter extends XQueryXConverter {
    ...
    public XPathXPrinter(XMLWriter xw) {
        super(null, xw);
    }
...
//    }
        ...
        switch (id) {
            ...
            case JJTPATHEXPR: {
                    ...
                    if (getParentID(node) != JJTSIMPLEMAPEXPR && node.jjtGetNumChildren() == 1) {
                        ...
                        if (only_child.id == JJTPOSTFIXEXPR && only_child.jjtGetNumChildren() == 1)
                        {
                            ...
                            this.transform(only_grandchild);
                            return true;
                        }
                    }
            ...
            case JJTRANGEEXPR: {
                xw.putStartTag(node, "xqx:rangeSequenceExpr");
                {
                    ...
                    this.transform(node.getChild(0));
                    xw.putEndTag(node);
                }
                {
                    xw.putStartTag(node, "xqx:endExpr");
                    this.transform(node.getChild(1));
                    xw.putEndTag(node);
                }
            ...
            case JJTGROUPINGSPEC: {
                ...
                this.transformChildren(node, i, i); i++;
            ...
            case JJTSWITCHCASECLAUSE: {
                ...
                for (int i = 0; i < nChildren - 1; i++) {
                   ...
                   this.transformChildren(node, i, i);
                   xw.putEndTag(node);
                }

                xw.putStartTag(node, "xqx:resultExpr");
                this.transformChildren(node, nChildren - 1);
                xw.putEndTag(node);
                xw.putEndTag(node);
            ...
            case JJTCONTEXTITEMDECL: {
                ...
                for (int i = 0; i < n; i++) {
                     ...
                     switch (child.id) {
                        ...
                        case JJTITEMTYPE: {
                           xw.putStartTag(child, "xqx:typeDeclaration");
                           this.transform(child);
                           xw.putEndTag(child);
                           break;
                        }
            ...
            case JJTDECIMALFORMATDECL: {
                ...
                if (child.id == JJTQNAME || child.id == JJTURIQUALIFIEDNAME) {
                   ...
                   // no QName, so it must be a default declaration
                    String[][] attributes = {{"xqx:default", "true"}};
                ...
                for (; i < n; i++) {
                   ...
                   this.transform(paramName);
                   this.transform(paramValue);
                   xw.putEndTag(child);
            ...
            case JJTTYPEDFUNCTIONTEST:
            {
                ...
                xw.putStartTag(node, "xqx:paramTypeList");
                   this.transformChildren(node, 0, nChildren - 2);
                xw.putEndTag(node);
    ...
    // namely the ones at positions 0 thru hi inclusive.
    {
        ...
        if (hi == 1) {
            ...
            this.transformChildren(sme_node, 0, 0);
        } else {
        ...
        // Right operand:
        this.transformChildren(sme_node, hi, hi);
    ...
    protected void filterPredicate(SimpleNode node, int start, int end) {
        ...
        if (predicates != 0) {
           xw.putStartTag(node, "xqx:predicates");
           this.transformChildren(node, end - predicates + 1, end);
           xw.putEndTag(node);
        }
